      ...
      <PageHeader
        ...
        actions={(
          <div className="flex items-center gap-2">
            <a href={`/api/csv-invoices/${row.id}/csv`} className="px-3 py-2 text-sm rounded-xl border border-white/30 bg-white/10 hover:bg-white/20 text-white">Download CSV</a>
            <EnhancedPdfButton
              id={row.id}
            ...
            <a href="/dashboard/csv-invoices" className="px-3 py-2 text-sm rounded-xl border border-white/30 bg-white/10 hover:bg-white/20 text-white">Back to list</a>
          </div>
        )}
      />
        ...
        {isEdit ? (
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="divide-y divide-gray-100">
              {entries.map(([k, v]) => (
                <tr key={k}>
                  <td className="px-4 py-2 whitespace-nowrap text-sm text-slate-500">{k.split('_').join(' ')}</td>
                  <td className="px-4 py-2 whitespace-nowrap text-sm text-slate-900">{String(v ?? '')}</td>
                </tr>
              ))}
            </tbody>
          </table>
